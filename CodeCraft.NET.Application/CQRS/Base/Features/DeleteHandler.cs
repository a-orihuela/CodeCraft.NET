...
namespace CodeCraft.NET.Application.CQRS.Base.Features
{
	...
	public abstract class DeleteHandler<TRequest, TEntity, TUnitOfWork> : BaseHandler<TRequest, TEntity, TUnitOfWork>, IRequestHandler<TRequest, bool>
		where TRequest : class, IRequest<bool>
		where TEntity : BaseDomainModel
		where TUnitOfWork : IUnitOfWork
	{
		public readonly string ERROR_MESSAGE = ResourceAccess.Messages("EntityDeleteError");
		public readonly string SUCCESS_MESSAGE = ResourceAccess.Messages("EntityDeleteSuccess");

		protected DeleteHandler(TUnitOfWork unitOfWork, IMapper mapper, ILogger logger)
		...
		public virtual async Task<bool> Handle(TRequest request, CancellationToken cancellationToken)
		{
			...
			if (await DeleteEntityFromRepository(entity) <= 0)
			{
		...
		private async Task<int> DeleteEntityFromRepository(TEntity entity)
		{
